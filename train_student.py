...
parser.add_argument('--window', type=str, default='hann', help='Hanning window -> hann or Not')
...
def train(epoch, model_t, model_s, optimizer, ema):
    ...
    for batch_idx, (x, y, c) in enumerate(train_loader):
        ...
        stft_student = stft(x_student[:, 0, 1:], window=args.window)
        stft_truth = stft(x[:, 0, 1:], window=args.window)
        loss_frame = criterion_frame(stft_student, stft_truth) * args.alpha
...
def evaluate(model_t, model_s, ema=None):
    ...
    for batch_idx, (x, y, c) in enumerate(test_loader):
        ...
        loss_frame = criterion_frame(stft_student, stft_truth.detach()) * args.alpha
